...
1. condition 안에서 useState를 사용
2. 훅은 linkedList 로 관리되어 순서가 중요한데 중간에 condition 등으로 순서가 보장이 안되어버리면 이상한 훅을 가져올 수 있음
3. 비슷한 사례가 딱히 생각나진 않음..
...
1. 객체를 찾아서 바꾸는것과 전체 리스트를 업데이트 하는것의 차이
2. 비교할때 리엑트는 리스트의 경우 참조가 달라지지 않으면 리렌더링을 안함
3. 메모리와 상태관리의 이점을 생각했을때 상태관리가 더 편한쪽이 나아서. js 의 GC와 불변의 안전함을 가져갈 수 있는 선택
4. React.memo 가 제대로 작동하려면 B안을 써야함. === 으로 비교를 수행하고 변경사항이 있을때 업데이트 되기 때문
...
1. 단순 업데이트의 경우 ( 1개 등 )
2. 선언형으로 상태만 바라보는 형태로 UI를 구축하기 위해서
3. C, A, B 순으로 빠름. 단순 DOM 업데이트 속도를 Virtual DOM 이 이길 수 없고 100개의 데이터를 DOM으로 직접 업데이트하는건 느림
4. 둘다 View를 재사용하는것은 같지만, Android 의 경우 업데이트가 있으면 그를 알려줘아하고 React 는 자동으로 상태를 보고 업데이트를 수행함
